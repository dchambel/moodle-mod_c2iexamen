...
//rev 311 ajout parametres optionels proxy définis dans Moodle
    ...
    if (!empty($CFG->proxyport)) $optionssoap['proxy_port' ]=$CFG->proxyport;
    ...
    $c2i=new C2IWS($wsdl,$uri,$optionssoap); //important
    return $c2i;
}
...
function c2i_getexamens($typep='positionnement') {
	...
	try{
		...
		$res=$c2i->get_examens($lr->getClient(),$lr->getSessionKey(),'positionnement');
		        	ws_error_log(print_r($res,true));
		if ($CFG->inclure_certification) {
    	       	$res2=$c2i->get_examens($lr->getClient(),$lr->getSessionKey(),'certification');
...
function c2i_inscritcandidats($candidats,$idexamen) {
	...
	try {
		$res=array();
		 $c2i=connect_to_plateforme();
        $lr=$c2i->login($CFG->login_plateforme,$CFG->passe_plateforme);
	 	$res[]=$c2i->inscrit_examen($lr->getClient(),$lr->getSessionKey(),$idexamen,$candidats,'login');
		$c2i->logout($lr->getClient(),$lr->getSessionKey());
		return  $res;
	} catch (Exception $e) {
		debugging($e->getMessage());
